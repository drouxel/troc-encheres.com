...
public class UtilisateurManager {
	...
	/**
	 * Permet d'ajouter un utilisateur √† la base de donn√©
	 * @param utilisateur
	...
	public void ajouterUtilisateur(Utilisateur u) throws BusinessException {
		...
		// insertion de l'utilisateur si tout les 'isValide' sont √† 'true'
		try {
			if(isValide) {
	...
	private boolean verifAjouterUtil(Utilisateur u, boolean isValide) throws BusinessException {
		BusinessException BExc = new BusinessException();
		// v√©rifier que tous les champs de saisies soit tous remplis
		if(u == null) {
			isValide = false;
			...
			isValide = false;
			BExc.ajouterErreur("Le champ PrÈnom est obligatoire !");
			throw BExc;
	...
	public void modifierUtilisateur(Utilisateur u) throws BusinessException {
		...
		boolean isValide = true;
				...
				// v√©rifie que 'isValide' soit 'true' avant de modifier
				try {
					if(isValide) {
	...
	/**
	 * m√©thode permettant de modifier utilisateur pr√©sent dans la base de donn√©es.
	 * @param u
	 * @throws BusinessException
	 */
	...
	public List<String> getPseudos() throws BusinessException {
		...
		try {
			...
			BExc.ajouterErreur("Erreur de rÈcupÈration du pseudo.");
		}
	...
	public List<String> getMails() throws BusinessException {
		...
		try {
			...
			BExc.ajouterErreur("Erreur de rÈcupÈration de l'adresse e-mail.");
		}
